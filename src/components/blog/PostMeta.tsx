...
/** Author (initials avatar + name) on the left, coral "Read more ›" on the right. */
export function PostByline({ author, href, linkLabel }: { author: string; href: string; linkLabel: string }) {
  return (
    <div className="flex items-center justify-between gap-3">
      <span className="flex items-center gap-2 text-[12px] font-medium lg:text-[max(12px,calc(var(--u)*13))]">
        <span
          aria-hidden
      ...
      <Link
        ...
        className="font-display text-[12px] font-bold uppercase tracking-[0.03em] text-coral hover:text-gold focus-visible:outline focus-visible:outline-2 focus-visible:outline-gold lg:text-[max(12px,calc(var(--u)*13))]"
      >
