      ...
      <ul className="mx-auto grid w-full max-w-[1480px] grid-cols-1 gap-10 sm:grid-cols-2 md:grid-cols-3 md:gap-[clamp(1.5rem,4.5vw,4.5rem)] lg:max-w-none lg:gap-x-[6.288%]">
        {cards.map((card) => (
          <li
            key={card.id}
            id={card.id}
            className="@container sm:last:col-span-2 sm:last:mx-auto sm:last:w-[calc(50%-1.25rem)] md:last:col-span-1 md:last:mx-0 md:last:w-auto"
          >
            {/* Figma card 381.593 x 643.424 — sizes live in ServiceCard (cqw). */}
