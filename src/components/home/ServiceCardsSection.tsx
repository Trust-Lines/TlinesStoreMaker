...
export function ServiceCardsSection({ cards }: ServiceCardsSectionProps) {
  return (
    <section aria-label="Store types we build" className="bg-cream px-5 py-12 sm:px-8 md:py-16 lg:px-[2.5%] lg:py-[72px]">
      ...
      <ul className="mx-auto grid w-full max-w-[1480px] grid-cols-1 gap-10 sm:grid-cols-2 md:grid-cols-3 md:gap-[clamp(1.5rem,4.5vw,4.5rem)]">
        {cards.map((card) => (
          <li
            key={card.id}
            className="sm:last:col-span-2 sm:last:mx-auto sm:last:w-[calc(50%-1.25rem)] md:last:col-span-1 md:last:mx-0 md:last:w-auto"
          >
